mjsCrypt = function() {
    ...
    function t(r, n) {
        var t = new Uint8Array(r.length + n.length);
        return t.set(new Uint8Array(r), 0), t.set(new Uint8Array(n), r.length), t
    }

    function e(r) {
        var n = 0;
        for (i = 0; i < r.length; i++) char = r.charCodeAt(i), n = (n << 5) - n + char, n &= n;
        return n
    }

    function o(r) {
        for (var t = u(), e = new Uint8ClampedArray(r), o = 0; o < e.length; o++) e[o] = Math.floor(256 * Math.random()) ^ t[n(o, t.length)];
        return e
    }
    ...
    function u() {
        for (var n = [window.navigator.userAgent, Date().toLocaleString(), y], t = [window.history, window.screen, window.location, window.navigator, window.location, window.menubar, window.scrollbars, window.statusbar], o = 0; o < t.length; o++) n.push(JSON.stringify(a(t[o])));
        r = [];
        for (var o = 0; o < n.length; o++) {
            var i = Math.abs(e(n[o]));
            r.push(255 & i), r.push((65280 & i) >> 8), r.push((16711680 & i) >> 16), r.push((4278190080 & i) >> 24)
        }
        var u = new Uint8Array(r);
        return c(r, r, 10, 0, u.length)
    }

    function c(r, t, e, o, a) {
        for (var i = 0, u = t.length, c = a, f = o, d = 0, h = 0, w = 0; e > w; w++) {
            for (var l = 0; c > l; l++) r[l + f] = (r[l + f] + t[n(i, u)] + l) % 256, r[l + f] = de[r[l + f]], i++;
            d = 0;
            for (var l = 0; c > l; l++) d = (d + t[n(l, u)] + l) % c, h = r[l + f], r[l + f] = r[d + f], r[d + f] = h;
            for (var l = 0; c - 1 > l; l++) r[l + f] = (r[l + f] + r[l + 1 + f]) % 256
        }
        return r
    }

    function f(r, t, e, o, a) {
        for (var i = t.length, u = a, c = o, f = e * u, d = 0, h = 0, w = 0, w = 0; u > w; w++) d = n(d + t[n(w, i)] + w, u);
        for (var l = d, g = 0; e > g; g++) {
            for (w = u - 2; w > -1; w--) r[w + c] = n(r[w + c] - r[w + c + 1], 256);
            for (d = l, w = u - 1; w > -1; w--) h = r[w + c], r[w + c] = r[d + c], r[d + c] = h, d = n(d - t[n(w, i)] - w, u);
            for (w = u - 1; w > -1; w--) f--, r[w + c] = dd[r[w + c]], r[w + c] = n(r[w + c] - t[n(f, i)] - w, 256)
        }
        return r
    }
